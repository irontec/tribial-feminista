...
/**
 * Data Mapper implementation for tribialfeminista\Model\KlearUsers
 *
 * @package tribialfeminista\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'tribialfeminista\\Model\\KlearUsers';
     ...
     *
     * @param tribialfeminista\Model\Raw\KlearUsers $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \tribialfeminista\Model\Raw\KlearUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \tribialfeminista\Model\Raw\KlearUsers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\tribialfeminista\Model\\KlearUsers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return tribialfeminista\\Mapper\\Sql\\DbTable\\KlearUsers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('tribialfeminista\\Mapper\\Sql\\DbTable\\KlearUsers');
        }
     ...
     *
     * @param tribialfeminista\Model\Raw\KlearUsers $model The model to delete
    ...
    public function delete(\tribialfeminista\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \tribialfeminista\Model\Raw\KlearUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\tribialfeminista\\Model\\KlearUsers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\tribialfeminista\\Model\\KlearUsers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('tribialfeminista\\Mapper\\Sql\\DbTable\\KlearUsers', $capitalizedFk);
     ...
     */
    public function save(\tribialfeminista\Model\Raw\KlearUsers $model)
    {
     ...
     *
     * @param \tribialfeminista\Model\Raw\KlearUsers $model
     * @param boolean $useTransaction Flag to indicate if save should be done inside a database transaction
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\tribialfeminista\Model\Raw\KlearUsers $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\tribialfeminista\Model\Raw\KlearUsers $model,
     ...
     * @param tribialfeminista\Model\Raw\KlearUsers|null $entry The object to load the data into, or null to have one created
     * @return tribialfeminista\Model\Raw\KlearUsers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \tribialfeminista\Model\KlearUsers();
        }
        ...
        } else if ($data instanceof \tribialfeminista\Model\Raw\KlearUsers) {
