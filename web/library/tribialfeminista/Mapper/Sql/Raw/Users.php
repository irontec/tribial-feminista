...
/**
 * Data Mapper implementation for tribialfeminista\Model\Users
 *
 * @package tribialfeminista\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'tribialfeminista\\Model\\Users';
     ...
     *
     * @param tribialfeminista\Model\Raw\Users $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \tribialfeminista\Model\Raw\Users) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \tribialfeminista\Model\Raw\Users object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\tribialfeminista\Model\\Users object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'fbId' => $model->getFbId(),
            'twId' => $model->getTwId(),
            'fbPicUrl' => $model->getFbPicUrl(),
            'twPicUrl' => $model->getTwPicUrl(),
            'fbUsername' => $model->getFbUsername(),
            'twUsername' => $model->getTwUsername(),
        );
     ...
     *
     * @return tribialfeminista\\Mapper\\Sql\\DbTable\\Users
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('tribialfeminista\\Mapper\\Sql\\DbTable\\Users');
        }
     ...
     *
     * @param tribialfeminista\Model\Raw\Users $model The model to delete
    ...
    public function delete(\tribialfeminista\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \tribialfeminista\Model\Raw\Users) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\tribialfeminista\\Model\\Users object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\tribialfeminista\\Model\\Users object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('tribialfeminista\\Mapper\\Sql\\DbTable\\Users', $capitalizedFk);
     ...
     */
    public function save(\tribialfeminista\Model\Raw\Users $model)
    {
     ...
     *
     * @param \tribialfeminista\Model\Raw\Users $model
     * @param boolean $useTransaction Flag to indicate if save should be done inside a database transaction
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\tribialfeminista\Model\Raw\Users $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\tribialfeminista\Model\Raw\Users $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getScores(null, null, true) !== null) {
     ...
     * @param tribialfeminista\Model\Raw\Users|null $entry The object to load the data into, or null to have one created
     * @return tribialfeminista\Model\Raw\Users The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \tribialfeminista\Model\Users();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setFbId($data['fbId'])
                  ->setTwId($data['twId'])
                  ->setFbPicUrl($data['fbPicUrl'])
                  ->setTwPicUrl($data['twPicUrl'])
                  ->setFbUsername($data['fbUsername'])
                  ->setTwUsername($data['twUsername']);
        ...
        } else if ($data instanceof \tribialfeminista\Model\Raw\Users) {
            $entry->setId($data->getId())
                  ...
                  ->setLoginType($data->getLoginType())
                  ->setFbId($data->getFbId())
                  ->setTwId($data->getTwId())
                  ->setFbPicUrl($data->getFbPicUrl())
                  ->setTwPicUrl($data->getTwPicUrl())
                  ->setFbUsername($data->getFbUsername())
                  ->setTwUsername($data->getTwUsername());
