...
/**
 * Data Mapper implementation for tribialfeminista\Model\Packages
 *
 * @package tribialfeminista\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class Packages extends MapperAbstract
{
    protected $_modelName = 'tribialfeminista\\Model\\Packages';
     ...
     *
     * @param tribialfeminista\Model\Raw\Packages $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \tribialfeminista\Model\Raw\Packages) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \tribialfeminista\Model\Raw\Packages object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\tribialfeminista\Model\\Packages object in toArray for " . get_class($this);
            }
     ...
     *
     * @return tribialfeminista\\Mapper\\Sql\\DbTable\\Packages
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('tribialfeminista\\Mapper\\Sql\\DbTable\\Packages');
        }
     ...
     *
     * @param tribialfeminista\Model\Raw\Packages $model The model to delete
    ...
    public function delete(\tribialfeminista\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \tribialfeminista\Model\Raw\Packages) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\tribialfeminista\\Model\\Packages object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\tribialfeminista\\Model\\Packages object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('tribialfeminista\\Mapper\\Sql\\DbTable\\Packages', $capitalizedFk);
     ...
     */
    public function save(\tribialfeminista\Model\Raw\Packages $model)
    {
     ...
     *
     * @param \tribialfeminista\Model\Raw\Packages $model
     * @param boolean $useTransaction Flag to indicate if save should be done inside a database transaction
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\tribialfeminista\Model\Raw\Packages $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\tribialfeminista\Model\Raw\Packages $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getQuestions(null, null, true) !== null) {
     ...
     * @param tribialfeminista\Model\Raw\Packages|null $entry The object to load the data into, or null to have one created
     * @return tribialfeminista\Model\Raw\Packages The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \tribialfeminista\Model\Packages();
        }
        ...
        } else if ($data instanceof \tribialfeminista\Model\Raw\Packages) {
