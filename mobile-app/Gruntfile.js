// Generated on 2015-01-13 using generator-ionic 0.6.1
'use strict';
...
module.exports = function (grunt) {
    ...
    ngconstant: {
      ...
      development: {
        constants: {
          ENV: 'development',
          API: {
            ENDPOINT: 'http://localhost/tribialfeminista/api/',
            FACEBOOK: {
              ENDPOINT: 'https://graph.facebook.com/v2.2/',
  ...
  grunt.registerTask('runProd', function() {
    grunt.config('concurrent.ionic.tasks', ['ionic:run:' + this.args.join(), 'watch']);
    return grunt.task.run(['initProd', 'concurrent:ionic']);
  });

  grunt.registerTask('buildProd', function() {
    return grunt.task.run(['initProd', 'ionic:build:' + this.args.join()]);
  });

  grunt.registerTask('initProd', [
    'clean',
    'ngconstant:production',
    'wiredep',
    'concurrent:server',
